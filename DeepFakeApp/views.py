...
def PredictAction(request):
    if request.method == 'POST':
        ...
        file_path = 'DeepFakeApp/static/' + name
        ...
        fake_prob = float(predict[0][0])
        real_prob = float(predict[0][1])

        fake_percent = round(fake_prob * 100, 2)
        real_percent = round(real_prob * 100, 2)
        ...
        if label == "Fake":
            ...
            context = {
                'data': f"""
                Predicted: {label}<br>
                Original Confidence: {real_percent}%
                """
            }
